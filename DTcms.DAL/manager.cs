...
namespace DTcms.DAL
{
    ...
    public partial class manager
    {
        ...
        public int Add(Model.manager model)
        {
            ...
            strSql.Append("role_id,role_type,user_name,user_pwd,real_name,telephone,email,is_lock,add_time,Dept_Code,Role_Code)");
            strSql.Append(" values (");
            strSql.Append("@role_id,@role_type,@user_name,@user_pwd,@real_name,@telephone,@email,@is_lock,@add_time,@deptcode,@rolecode)");
            ...
            SqlParameter[] parameters = {
					...
					new SqlParameter("@rolecode", SqlDbType.NVarChar,26)};
            ...
            parameters[9].Value = model.DeptInfo.Dept_Code;
            parameters[10].Value = model.RoleInfo.Role_Code;
        ...
        public bool Update(Model.manager model)
        {
            ...
            strSql.Append(",Role_Code=@rolecode");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@role_id", SqlDbType.Int,4),
					new SqlParameter("@role_type", SqlDbType.Int,4),
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,role_id,role_type,user_name,user_pwd,real_name,telephone,email,is_lock,add_time,Dept_Code,Role_Code from dt_manager ");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                model.DeptInfo.Dept_Code = ds.Tables[0].Rows[0]["Dept_Code"] + "";
                //model.DeptInfo.Dept_Name = ds.Tables[0].Rows[0]["Dept_Name"] + "";
                model.RoleInfo = new Model.user_roleinfo();
                model.RoleInfo.Role_Code = ds.Tables[0].Rows[0]["Role_Code"] + "";
                //model.RoleInfo.Role_Name = ds.Tables[0].Rows[0]["Role_Name"] + "";
                return model;
            }
            else
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,role_id,role_type,user_name,user_pwd,real_name,telephone,email,is_lock,add_time,Dept_Code,Role_Code ");
        ...
        public DataSet GetList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" id,role_id,role_type,user_name,user_pwd,real_name,telephone,email,is_lock,add_time,Dept_Code,Role_Code ");
            strSql.Append(" FROM dt_manager ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
        ...
        public string GetTitleByCode(string code)
        {
            ...
            if (code.Trim() != "")
            {
                strSql.Append(" where user_name='" + code + "'");
            }
            DataTable tb = DbHelperSQL.Query(strSql.ToString()).Tables[0];
            if (tb != null && tb.Rows.Count > 0) { value = tb.Rows[0]["real_name"] + ""; }
            return value;
        }
