...
{
    public partial class bridge_cost_report : Web.UI.ManagePage
    {
        ...
        //按照月份查询
        public void exportReportForMonth()
        {
            string stime = txtStartTime.Text;
            string etime = txtEndTime.Text;
            ...
            for (int i = 0; i < total_months; i++)
            {
                ...
                where += "YEAR(Return_Time) = '" + year + "' and ";
                where += "MONTH(Return_Time) = '" + month + "'";
                DataTable items = new BLL.car_use_recordbll().GetCarUseCost(0, where, "Return_Time").Tables[0];
            ...
            DataToExcel.DataTableToExcelForStyle(dt, list, "bridge_cost2.xls", "车辆路桥费用累总",
                 false, false, false, false, 1, 1, 1, 6);

            outSuccess("Y", "bridge_cost2.xls");
            #endregion
        ...
        public void exportReport()
        {
            ...
            for (int i = 0; i < tbUseCost.Rows.Count; i++)
            {
                ...
                for (int j = 0; j < tbcarcosted.Rows.Count; j++)
                {
                    ...
                    //string dricode = tbcarcosted.Rows[j]["BridgeCost"] + "";
                    ...
                    if (utime == time)
                    {
                        ...
                        if (deptpros == "10001")
                        {
                            ...
                            decimal brcost = 0;
                            string sbrcost = tbcarcosted.Rows[j]["BridgeCost"] + "";
            ...
            DataToExcel.DataTableToExcel(dt, list, "bridge_cost.xls", "车辆过桥费汇总表");

            outSuccess("Y","bridge_cost.xls");
        }
    }
}