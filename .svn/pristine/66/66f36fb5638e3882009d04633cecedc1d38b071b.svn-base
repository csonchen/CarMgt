<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="oil_cost_report.aspx.cs" Inherits="DTcms.Web.admin.report.oil_cost_report" %>
...
<script type="text/javascript">
    ...
    function export_click() {
        ...
        $.ajax({
            url: "oil_cost_report.aspx?task=exportReportForMonth",
            data: {
                "stime": strstime,
                "etime": stretime,
            ...
            success: function (result) {
                ...
                //}
                if (result.IsOK == "Y") {
                    window.open( "../../TEMP/" + result.Message);
                } else {
...
<form id="form1" runat="server">
    <div class="navigation">首页 &gt; 报表管理 &gt; 车辆燃油费汇总列表</div>
    <div class="tools_box">
	    <div class="tools_bar">
            <div style="margin-left:10px;margin-top:10px;float:left">时间：
                <asp:TextBox ID="txtStartTime" runat="server" CssClass="txtInput normal dateISO" maxlength="20" Width="100px" onfocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy年MM月',minDate:'#F{$dp.$D(\'txtEndTime\',{M:-29})}',maxDate:'#F{$dp.$D(\'txtEndTime\')||\'%y-%M\'}'})"></asp:TextBox>
			    <asp:TextBox ID="txtEndTime" runat="server" Width="100px" onfocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy年MM月',minDate:'#F{$dp.$D(\'txtStartTime\')}',maxDate:'#F{$dp.$D(\'txtStartTime\',{M:29})||\'%y-%M\'}'})" CssClass="txtInput normal dateISO"></asp:TextBox>
                <asp:DropDownList id="ddlDept_Pros" CssClass="select2 required" runat="server">
                        ...
                        <asp:ListItem Value="10000" Selected="True">总办+行政</asp:ListItem>
                    </asp:DropDownList>
            </div>
           ...
           <%-- <a href="car_type_edit.aspx?action=<%=DTEnums.ActionEnum.Add %>" class="tools_btn"><span><b class="add">添加车辆燃油费汇总</b></span></a>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="msgtable">
      <tr>
        <th>选择</th>
        <th align="left">车辆燃油费汇总编码</th>
        <th align="left">车辆燃油费汇总名称</th>
        <th>操作</th>
      </tr>
    </HeaderTemplate>
    <ItemTemplate>
