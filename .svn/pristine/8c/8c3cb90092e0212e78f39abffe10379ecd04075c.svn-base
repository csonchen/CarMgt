...
namespace DTcms.Web.admin.cars.car_use_record
{
    public partial class car_use_record_edit : Web.UI.ManagePage
    {
        ...
        protected void ddlDept_Code_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //使用车人选项生效
            this.ddlUser_Code.Enabled = true;
        }
        ...
        {
            string carNumber = ddlCar_Number.SelectedValue;
            BLL.carbll carbll = new BLL.carbll();
            
            //根据车牌号求驾驶员编号Driver_Code
            Model.carinfo car = carbll.GetModel(carNumber, "");
            string driverCode = car.Driver.Driver_Code;
            //绑定驾驶员表
            BindDriver_Code(" Driver_Code='" + driverCode + "'");
            //选中第一项
            if (ddlDriver_Code.Items.Count == 2 )
            {
                ddlDriver_Code.Items[1].Selected = true;
            }
        }

        //根据驾驶员选择车牌（一对一）
        protected void ddlDriver_Code_SelectedIndexChanged(object sender, EventArgs e)
        {
            string driverCode = ddlDriver_Code.SelectedValue;
            
            //绑定车辆信息表
            BindCar_Number(" Driver_Code='" + driverCode + "'");
            //选中第一项
            if (ddlCar_Number.Items.Count == 2)
            {
                ddlCar_Number.Items[1].Selected = true;
            }
        }
