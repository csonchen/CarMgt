...
{
    public partial class accident_record_list : Web.UI.ManagePage
    {
        ...
        protected string CombSqlTxt(string _keywords)
        {
            ...
            {
                strTemp.Append(" and (Car_Number like '%" + _keywords + "%' or Type_Code like '%" + _keywords + "%')");
            }
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("accident_record_list_page_size"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("accident_record_list.aspx", "group_id={0}&keywords={1}",
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("accident_record_list_page_size", _pagesize.ToString(), 43200);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("accident_record_list.aspx", "group_id={0}&keywords={1}",
